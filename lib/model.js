...
class Model {
  ...
  */
   ...
   * @param  {Array}          [options.updateOnDuplicate]      Fields to update if row key already exists (on duplicate key update)? (only supported by MySQL, MariaDB, MSSQL, SQLite >= 3.24.0 & Postgres >= 9.5).
   * @param  {Transaction}    [options.transaction]            Transaction to run query under
   * @param  {Function}       [options.logging=false]          A function that gets executed while running the query to log the sql.
   * @param  {boolean}        [options.benchmark=false]        Pass query execution time in milliseconds as second argument to logging function (options.logging).
  ...
  static async bulkCreate(records, options = {}) {
    ...
    const recursiveBulkCreate = async (instances, options) => {
      ...
      if (options.updateOnDuplicate && !this.sequelize.dialect.supports.inserts.updateOnDuplicate) {
