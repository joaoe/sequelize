...
class MSSqlQueryInterface extends QueryInterface {
  ...
  */
  ...
  async upsert(tableName, insertValues, updateValues, where, options) {
    ...
    const uniqueKeyColumns = this._columnsOfUniqueKeys(model);
  ...
  _columnsOfUniqueKeys(model) {
    return [
      ...Object.values(model.uniqueKeys).map(item => item.fields),
      ...Object.values(model._indexes).filter(item => item.unique).map(item => item.fields),
    ];
  }
}
