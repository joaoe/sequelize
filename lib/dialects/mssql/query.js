...
class Query extends AbstractQuery {
  ...
  isCallQuery() {
    return this.sql.split(/\s+/, 1)[0].toLowerCase() === 'exec';
  }
  ...
  formatResults(data, rowCount) {
    ...
    if (this.isSelectQuery() || this.isCallQuery()) {
  ...
  formatError(err, errStack) {
    ...
    match = match || err.message.match(/Cannot insert duplicate key row in object .* with unique index '(.*)'.(:? The duplicate key value is \((.*)\).)?/);

    if (match && match.length > 1) {
      let fields = {};
      const [uniqueKey, uniqueFields] = this._getUniqueKeyFromModel(this.model, match[1]);

      let message = 'Validation error';

      if (uniqueKey && Boolean(uniqueKey.msg)) {
        message = uniqueKey.msg;
      } else if (uniqueFields.length) {
        message = `Unique constraint violation on (${uniqueFields.join(',')})`;
      }

      if (match[3]) {
        const values = match[3].split(',').map(part => part.trim());
        if (uniqueKey) {
          fields = _.zipObject(uniqueFields, values);
        } else {
    ...
    if (this.instance) {
      ...
      if (this.instance.dataValues) {
        for (const key in results[0]) {
          if (key !== '$action' && Object.prototype.hasOwnProperty.call(results[0], key)) {
