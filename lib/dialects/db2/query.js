...
class Query extends AbstractQuery {
  ...
  formatError(err, errStack, conn, parameters) {
    ...
    if (match && match.length > 0) {
      ...
      let [uniqueKey, uniqueFields] = this._getUniqueKeyFromModel(this.model, uniqueIndexName);

      if (!uniqueKey && this.options.fields) {
        uniqueKey = this.options.fields[match[1] - 1];
        uniqueFields = [uniqueKey.column];
      }

      for (const column in uniqueFields) {
        if (this.options.where
          && this.options.where[column] !== undefined) {
          fields[column] = this.options.where[column];
        } else if (this.options.instance && this.options.instance.dataValues
          && this.options.instance.dataValues[column]) {
          fields[column] = this.options.instance.dataValues[column];
        } else if (parameters) {
          fields[column] = parameters['0'];
        }
      }
