...
class Db2QueryInterface extends QueryInterface {
  ...
  async upsert(tableName, insertValues, updateValues, where, options) {
    ...
    function getFieldName(field) {
      return typeof field === 'string' ? field : field.name || field.attribute || field;
    }

    // Lets combine unique keys and indexes into one
    indexes = _.map(model.uniqueKeys, value => {
      return value.fields.map(getFieldName);
    });

    for (const value of model._indexes) {
      if (value.unique) {
        // fields in the index may both the strings or objects with an attribute property - lets sanitize that
        indexes.push(value.fields.map(getFieldName));
      }
    }

    if (!_.isEmpty(options.conflictFields)) {
      indexes.push(options.conflictFields);
    }
