...
class AbstractQuery {
  ...
  /**
   * Helper method for formatError() and potentially others.
   * It finds the unique key or index of the given model by its name, OR given by its fields.
   * To be used for errors where unique constraints fail.
   *
   * @param {object} model An object/table model
   * @param {string} name The unique index/key name
   * @returns {Array} Two values [uniqueKey, fields], where uniqueKey is the uniqueKey object,
   *         and fields is an array of Strings with all columns in the unique index or key ready
   *         for display.
   * @private
   */

  _getUniqueKeyFromModel(model, name, fields = null, fieldsSubset = false) {
    const _getFields = obj => obj && obj.fields && obj.fields.map(f => f.name || f.attribute || f) || [];
    const _hasFields = !fields ? _ => false
      : fieldsSubset
        ? obj => _.some(_getFields(obj), v => fields.includes(v))
        : obj => _.isEqual(_getFields(obj), fields);

    let obj = null;
    if (model && model.uniqueKeys) {
      if (name && name in model.uniqueKeys) {
        obj = model.uniqueKeys[name];
      }

      if (!obj && fields) {
        obj = _.find(Object.values(model.uniqueKeys), _hasFields);
      }
    }

    if (!obj && model && model._indexes) {
      obj = _.find(model._indexes, x => x.unique && (x.name === name || _hasFields(x)));
    }

    return [obj, _getFields(obj)];
  }
  ...
  getUniqueConstraintErrorMessage(field) {
    const [uniqueKey] = this._getUniqueKeyFromModel(this.model, null, [field.replace(/"/g, '')], true);
    if (uniqueKey && uniqueKey.msg) {
      return uniqueKey.msg;
    }

    return field ? `${field} must be unique` : 'Must be unique';
  }
